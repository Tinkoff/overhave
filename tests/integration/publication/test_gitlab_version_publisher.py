...
class TestGitlabVersionPublisher:
    ...
    def test_none_context(
        self, gitlab_version_publisher_without_context: GitlabVersionPublisher, test_draft: DraftModel
    ) -> None:
        with count_queries(1):
            draft_status = gitlab_version_publisher_without_context.publish_version(draft_id=test_draft.id)
